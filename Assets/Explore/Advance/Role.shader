Shader "Explore/Role"
{
	Properties
	{
		...
		_Gloss("Gloss", Range(1, 8)) = 1
		_RampMap("Ramp Map", 2D) = ""{}
		_SpecRampMap("Spec Ramp", 2D) = ""{}
	}
	SubShader
	{
		...
		float Dggx(float nh, float roughness)
		{
			...
			return a2 / (UNITY_PI * (pow2(t) + 1e-7f));
		}
		...
		fixed4 frag (v2f i) : SV_Target
		{
			...
			fixed4 spec = tex2D(_SpecMap, i.uv) * _SpecScale;
			...
			float diffTerm = nl;
			float specTerm = pow(nh, _Gloss);
			//shadow and atten
			UNITY_LIGHT_ATTENUATION(atten, i, pos);

			fixed4 ramp = tex2D(_RampMap, float2(diffTerm, diffTerm));
			fixed4 specRamp = tex2D(_SpecRampMap, float2(specTerm, specTerm));

			fixed3 diffuse = albedo * ramp * atten;
			fixed3 specular = spec.r * _LightColor0.rgb * specRamp * atten;
